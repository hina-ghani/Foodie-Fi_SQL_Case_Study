...
WITH customersCount AS (
    SELECT COUNT(*) AS customersNum
    FROM (
        SELECT customer_id
        FROM subscriptions AS s
...
SELECT ROUND((customersNum * 100.0) / (SELECT COUNT(*) FROM subscriptions), 0) AS churn_percentage
FROM customersCount;
...
SELECT COUNT(DISTINCT s1.customer_id) AS downgraded
